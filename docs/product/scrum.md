...
```text
1. 工作项自己的开始与结束日期（预留，当前模型没有这两个字段）
2. 否则取它所属 Sprint 的起止日期
3. 否则归入「未排期」分组，不画条
```

首版只有第二和第三条生效：工作项目前不带自己的日期，时间感全部来自它所在的 Sprint。第一条是给后续留的位置——真要加这两个字段，也是因为个别事项需要脱离 Sprint 栅格单独排期，而不是因为 Timeline 画不出来。

没有这条降级链，Timeline 在真实项目里会是一片空白：Scrum 的工作项本来就不填日期，硬要求填才能上时间轴，等于要求团队维护两套排期。
...
**List** 是属性投影：筛选、排序、批量修改和导出的入口，也是完整工作项模型的入口。它是其余三个视图的数据底座，因此优先级最高，但不把所有字段画成等权的表格列：

- 第一层用于扫描，显示编号、标题、状态、优先级、负责人、Sprint、估算和更新时间；
- 类型、类别、父项、标签、阻塞、依赖及验收进度跟随标题显示为上下文和风险信号；
- 描述、验收标准内容、阻塞原因、关系维护及类型特有字段进入侧边详情；
- CSV 导出仍包含完整字段，避免紧凑的日常界面限制数据分析。

搜索和高频条件始终可见，完整分类条件按需展开。筛选结果先给出工作项、阻塞、未指派和未估算数量，帮助用户判断当前集合是否需要处理；这些都是当前查询的真实计数，不推算无法测量的进度。
...
Activity 也补不上这一课：它记录谁在什么时候改了哪条数据的第几版，不记录字段取值，所以查得到「这条事项是第三天被排进来的」，查不到「当时它估了几点」。

因此 Sprint 启动时写入一条承诺基线——当时的事项集合与点数合计——此后每天追加一条剩余量，写完都不再修改。基于它：

- 范围变化是当前事项集合与基线的差集，进出双向都能说清；
- 燃尽图的起点是基线点数，曲线其余的点来自每日记录。

基线记的是启动那一刻的事实，不是当前归属，因此它不与「Sprint 不保存工作项列表」冲突：成员关系仍然只由工作项自己的 Sprint 字段表达。
