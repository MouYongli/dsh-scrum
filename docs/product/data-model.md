...
|---|---|---|
| `feature` | 功能需求 | Story |
| `nfr_visible` | 用户能感受到的非功能需求，例如"页面三秒内加载完" | Story |
| `nfr_constraint` | 用户感受不到的约束，例如日志留存期限、合规要求 | Task |
| `tech_debt` | 技术债与重构，对外行为不变 | Task |
| `spike` | 探针，产出是一个结论而不是一个功能 | Task |
| `ops` | 运维与迁移 | Task |
| `docs` | 文档 | Task |
| `defect` | 已交付的功能不符合预期 | Bug |

分辨 Story 和 Task 的判据是"用户是否感受得到，以及能不能独立交付价值"。但这只是**创建时的推荐**，不是校验：选了 `tech_debt`，界面会预选 Task，团队想改成 Story 也照样可以。像"页面三秒内加载完"这样的边界案例，不同团队本来就会落到不同一侧，把判据变成硬性校验，只会让人卡在一个填不下去的表单前。

`category` 用固定取值而不是随便写的标签，是为了让"这轮 Sprint 有多少点花在技术债上"这类问题有答案。自由标签拼写不一、各项目各写各的，统计不出来；而这恰恰是 `labels` 存在的位置——它不需要可比，只需要好用。

没有人分类过的事项，`category` 是 `null`，这是一个正常取值而不是待修的数据。给它一个默认类别看起来更整齐，但那等于往上面那个统计里掺猜测——报一个更小的数字、同时报出"还有多少条没分类"，比报一个说不清成分的数字有用。
...
### 7.6 承诺基线：唯一一件必须在启动时记下来的事

7.5 说进度都是算出来的，有一件事是例外：**Sprint 启动那一刻承诺了哪些事项**。

它必须在启动时记下来，因为事后算不出来。看一条现在属于 sprint-12 的工作项，没有任何字段能告诉你它是启动时就在里面，还是第三天临时塞进去的。Activity 也帮不上忙：它记的是"谁在什么时候改了哪条数据的第几版"，不记字段取值，所以能查到"这条事项是第三天被排进来的"，却查不到"当时它估了几点"。

而这一个事实同时决定了两件事：

- **范围变化**："Sprint 启动后新增了 2 项"——当前成员集合减去基线，就是加进来的；反过来减，就是被拿走的；
- **燃尽图的起点**：曲线从承诺的点数开始往下走。没有基线，起点就只能用"现在这些事项加起来是多少"，而那个数字会随着中途加活儿一起上涨，于是一条越加越多的 Sprint 反而显得进展顺利。

所以 Sprint 启动时写入一条基线记录（当时的事项集合与点数合计），此后每天追加一条剩余量。这些记录写完就不再改。
