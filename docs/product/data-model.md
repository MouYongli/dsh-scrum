...
└── Project                     一个 Scrum 项目
    ...
    ├── WorkItem                一条工作项。Epic、Story、Task、Bug 都是它，靠 type 区分
...
Epic、Story、Task、Bug 不是四张表，而是同一个实体的 `type` 字段的四个取值。它们共享全部字段和全部规则，区别只在语义和界面呈现。
...
这个层级由 `parentId` 表达，是推荐用法而不是强制约束：系统只要求父子关系不成环、不跨项目，不检查 Epic 之下必须是 Story。产品设计中提到的 Feature 和 Spike 是后续新增项，当前版本没有。
...
| `rank` | 字符串 | 数字与小写字母，不以 `0` 结尾，至多 64 字符 | Backlog 排序键，见 6.6 |
| `blockedReason` | 字符串或 `null` | 至多 500 字符；`null` 表示未阻塞 | 阻塞原因，见 6.7 |
| `labels` | 字符串列表 | 至多 20 个，每个至多 40 字符；自动转小写并去重 | 标签 |
| `acceptanceCriteria` | 对象列表 | 至多 50 条 | 验收标准，见 6.4 |
...
**为什么没有 `blocked` 字段？**
它可以从 `blockedReason` 推出来。两个必须一致的字段迟早会不一致。见 6.7。
...
**为什么 `rank` 是字符串？**
为了让任意两条之间总能插进一条新的，从而一次拖拽只重写一个文件。见 6.6。
