...
| 声明范围 | `^0.1.0-rc.7`（即 `>=0.1.0-rc.7 <0.2.0`） | `peerDependencies` 与运行时检测共用 |
...
静态 `peerDependencies` 只在安装时生效，用户完全可以在 Profile 里装上超出范围的 Harness。因此插件在加载时再检测一次：读取实际解析到的 Harness 包版本，超出声明范围时拒绝加载，并在错误信息中同时给出实际版本和支持范围。
