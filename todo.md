...
- **B38. `BacklogQuery` 用 `sprintId` 三态，不用 `planned` 布尔**（#52）。看板要问「哪个 Sprint」，布尔答不上来；`null` = 产品 Backlog，缺省 = 全部。
- **B39. 看板列 = 领域的 `BOARD_STATUSES` ＋ `done`**（#52）。`BOARD_STATUSES` 是 WIP 视图（WIP 上限数的就是它），故意不含 done；看板必须画 done，因为那是工作被移动过去的地方。顺序仍从领域取，不在 UI 里另立一份。
- **B40. 看板进度由屏幕上的卡片聚合，不另取一次 `SprintProgress`**（#52）。Application 那份是 Agent 侧的权威；看板要的是「我正在看的这些卡片」的合计，另取一次会在两次读之间和卡片对不上 —— 而那恰好是人看看板要确认的东西。
- **B41. 移动卡片用带标签的下拉，不用拖拽**（#52）。拖拽键盘不可达；下拉还顺带说明这张卡能去哪几列，而拖拽目标要等指针悬上去才显形。
- **B42. Sprint 日期按 UTC 零点解释**（#52）。日期输入只给出「哪一天」，时区若取浏览器本地，同一个用户换台机器打开就会看到边界漂移。
- **B43. 关闭 Sprint 时未完成项的去向不预选**（#52）。预选等于替用户签字；关闭按钮在每一项都有答案前保持禁用，这与用例的拒绝规则是同一条，只是提前表达成按钮状态。
...
- **B55. 验收套件放在 Bundle 里**（#55）。Bundle 是唯一对外可安装单元，也是依赖树的顶点，只有它能同时 import Host、Agent Tools、UI 和 Community 组合。放在别处会让组合层反过来依赖 UI。
- **B56. 验收只 stub Harness Context 一样东西**（#55）。底下全是 Bundle 真正装的代码，跑在真实临时目录上；stub 的那一样正是 A14 说明还读不到的那一样。
- **B57. 「凭证不进 `.scrum/`」当成可断言的发布标准**（#55）。扫过一次完整运行写下的每个文件：没有可能存放密钥的字段名、没有 Workspace 原始路径、Activity 只引用会话不携带内容。
